...
test.addImport("import SHAObj from './dist/sha-obj-bex.esm.min.js';");
...
test.makeUnit(
    ...
    async (input) => {
        const shaObj = await SHAObj.new("SHA-1", input);
...
test.makeUnit(
    ...
    async (input) => {
        const shaObj = await SHAObj.new("SHA-256", input);
        return shaObj.hexdigest();
    },
    INPUT
);
...
test.makeUnit(
    ...
    async (input) => {
        const shaObj = await SHAObj.new("SHA-384", input);
        return shaObj.hexdigest();
    },
    INPUT
);
...
test.makeUnit(
    ...
    async (input) => {
        const shaObj = await SHAObj.new("SHA-512", input);
        return shaObj.hexdigest();
    },
    INPUT
);
...
test.makeUnit(
    ...
    "2ef7bde608ce5404e97d5f042f95f89f1c232871",
    async () => {
        const shaObj = await SHAObj.new("SHA-1", "Hello");
